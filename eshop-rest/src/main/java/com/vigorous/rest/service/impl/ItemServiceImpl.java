...
import com.vigorous.pojo.TbItemParamItemExample;
import com.vigorous.pojo.TbItemParamItemExample.Criteria;
...
public class ItemServiceImpl implements ItemService {
	...
	@Override
	public ResultModel getItemBaseInfo(long itemId) {
		try {
			//添加缓存逻辑
			//从缓存中取商品信息，商品id对应的信息
	...
	@Override
	public ResultModel getItemDesc(long itemId) {
		//添加缓存
		try {
			//添加缓存逻辑
			//从缓存中取商品信息，商品id对应的信息
			...
			if (!StringUtils.isBlank(json)) {
				...
				return ResultModel.ok(itemDesc);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	@Override
	public ResultModel getItemParam(long itemId) {
		//添加缓存
		try {
			//添加缓存逻辑
			//从缓存中取商品信息，商品id对应的信息
			...
			if (!StringUtils.isBlank(json)) {
				...
				return ResultModel.ok(paramItem);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		return new ResultModel(400, "无此商品规格");
	}
}
