...
import com.vigorous.home.pojo.ItemInfo;
...
public class ItemServiceImpl implements ItemService {
	...
	public TbItem getItemById(Long itemId) {
		...
		try {
			...
			if (!StringUtils.isBlank(json)) {
				ResultModel taotaoResult = ResultModel.formatToPojo(json, ItemInfo.class);//注意这里
	...
	public String getItemDescById(Long itemId) {
		try {
			...
			//转换成java对象
			ResultModel taotaoResult = ResultModel.formatToPojo(json, TbItemDesc.class);
			if (taotaoResult.getStatus() == 200) {
	...
	public String getItemParam(Long itemId) {
		try {
			...
			//把json转换成java对象
			ResultModel taotaoResult = ResultModel.formatToList(json, TbItemParamItem.class);
			if (taotaoResult.getStatus() == 200) {
